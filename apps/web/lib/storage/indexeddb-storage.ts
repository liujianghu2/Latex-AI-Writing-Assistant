...
function getBlobKey(name: string, id: string): string {
  return `${name}:${id}`;
}

async function migrateLegacyState(
  db: IDBPDatabase<OpenPrismDB>,
  name: string,
): Promise<PersistedState | null> {
  const legacy = await db.get("documentState", "state");
  if (!legacy) return null;

  const files = await Promise.all(
    legacy.files.map(async (file) => {
      if (file.type === "image") {
  ...
  await indexedDBStorage.setItem(name, JSON.stringify({ state }));

  const tx = db.transaction(["documentState", "blobs"], "readwrite");
  await tx.objectStore("documentState").delete("state");
  const blobStore = tx.objectStore("blobs");
  for (const file of legacy.files) {
    if (file.type === "image") {
      await blobStore.delete(file.id);
    }
  }
...
export const indexedDBStorage: StateStorage = {
  async getItem(name: string): Promise<string | null> {
    ...
    const stored = await db.get("documentState", name);
    if (!stored) {
      const legacyState = await migrateLegacyState(db, name);
      if (!legacyState) return null;
      return JSON.stringify({ state: legacyState });
    }

    const files = await Promise.all(
      stored.files.map(async (file) => {
        if (file.type === "image") {
          const blob = await db.get("blobs", getBlobKey(name, file.id));
          if (blob && typeof blob.data === "string") {
            return { ...file, dataUrl: blob.data };
          }
        }
    ...
    const pdfBlob = await db.get("blobs", getBlobKey(name, "pdf"));
    const pdfData =
      pdfBlob && pdfBlob.data instanceof Uint8Array ? pdfBlob.data : null;
  ...
  async setItem(name: string, value: string): Promise<void> {
    ...
    const ownedBlobIds = existingBlobs.filter(
      (id) => typeof id === "string" && id.startsWith(`${name}:`),
    ) as string[];
    const newBlobIds = new Set(blobsToStore.map((b) => b.id));
    const orphanIds = ownedBlobIds.filter((id) => !newBlobIds.has(id));
    ...
    if (!state.pdfData) {
      await blobStore.delete(getBlobKey(name, "pdf"));
    }
  ...
  async removeItem(name: string): Promise<void> {
    const db = await getDB();
    const tx = db.transaction(["documentState", "blobs"], "readwrite");
    await tx.objectStore("documentState").delete(name);
    const blobStore = tx.objectStore("blobs");
    const existingBlobs = await blobStore.getAllKeys();
    for (const id of existingBlobs) {
